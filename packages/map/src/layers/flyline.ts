...
export interface FlyLineOptions { from: [number, number]; to: [number, number]; color?: string; height?: number; segments?: number; speed?: number }
...
export class FlyLineLayer {
  ...
  addFlyLine(options: FlyLineOptions, latLngToPos: (lat: number, lng: number) => THREE.Vector3): THREE.Line {
    ...
    const material = new THREE.LineBasicMaterial({ color: options.color || '#00f5ff', transparent: true, opacity: 0.8 })
    const line = new THREE.Line(geometry, material)
    this.lines.add(line)
    return line
  }

  clear(): void { while (this.lines.children.length) this.lines.remove(this.lines.children[0]) }
